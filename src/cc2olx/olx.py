...
class OlxExport:
    ...
    def _create_olx_nodes(self, element_data: dict) -> List["xml.dom.minidom.Element"]:
        ...
        if not idref:
            ...
            console_logger.warning("A resource processing is skipped because it has no an identifier.")
            return self._fallback_olx_nodes

        resource = self.cartridge.define_resource(idref)
        if resource is None:
            self._export_statistics.failure_count += 1
            console_logger.warning("Missing resource: %s", idref)
            self._file_logger.warning("Missing resource: %s", idref)
            return self._fallback_olx_nodes
        ...
        self._export_statistics.failure_count += 1
        console_logger.warning('The resource with "%s" identifier value is not supported.', idref)
        self._file_logger.warning('The resource with "%s" identifier value is not supported.', idref)
        return self._fallback_olx_nodes
    ...
    def _post_process(self, olx_node: "xml.dom.minidom.Element", idref: str) -> None:
        ...
        for post_processor in self._content_post_processors:
            try:
                ...
                self._log_olx_node_processing_by_content_post_processor_failure(
                    idref,
                    olx_node.tagName,
                    post_processor,
                    exc,
    ...
    def _log_olx_node_processing_by_content_post_processor_failure(
        self,
        idref: str,
        tag_name: str,
        content_post_processor: AbstractContentPostProcessor,
        exception: Exception,
    ) -> None:
        """
        Log an error occurred during node processing by content post processor.
        """
        content_post_processor_type_name = type(content_post_processor).__name__

        console_logger.error(
            'An error occurred during <%s> node post-processing by %s for resource "%s":',
            tag_name,
            content_post_processor_type_name,
            idref,
        )
        console_logger.exception(exception)
        console_logger.error("The post processor is skipped.")

        self._file_logger.error(
            (
                'An error occurred during <%s> node post-processing by %s for resource "%s": "%s". The post processor '
                "is skipped."
            ),
            tag_name,
            content_post_processor_type_name,
