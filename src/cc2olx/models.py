...
class ResourceFile:
    ...
    def __repr__(self):
        return "<ResourceFile href={href} />".format(href=self.href, )
...
class Cartridge:
    ...
    def __repr__(self):
        ...
        text = '<{_class} version="{version}" file="{filename}" />'.format(
            _class=type(self).__name__, version=self.version, filename=filename,
        )
        return text
    ...
    def get_run_xml(self):
        ...
        """
            ...
            .format(title=self.get_title(), language=self.get_language(), )
            .strip()
        )
        return text
    ...
    def get_title(self):
        ...
        title = (
            self.metadata.get("lom", {}).get("general", {}).get("title")
            or "Default Course Title"
        )
        return title
    ...
    def _parse_general(self, node):
        ...
        if general:
            ...
            data["language"] = self._parse_text(
                general, "lomimscc:language/lomimscc:string"
            )
            data["description"] = self._parse_text(
                general, "lomimscc:description/lomimscc:string"
            )
    ...
    def _parse_rights(self, node):
        ...
        if element:
            data["is_restricted"] = self._parse_text(
                element, "lomimscc:copyrightAndOtherRestrictions/lomimscc:value",
            )
            data["description"] = self._parse_text(
                element, "lomimscc:description/lomimscc:string",
            )
            # TODO: cost
        return data
